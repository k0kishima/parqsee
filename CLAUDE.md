...
| `profile_query_column` | `(resultId, columnIndex, filter?, requestId?)` → `ColumnProfile` | The profile of one column of a kept result, over the rows `filter` keeps. The column is named by position; the rows are the ones the webview was given, never the whole result of the query |
