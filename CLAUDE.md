...
| `profile_column_counts` | `(path, column, filter?, requestId?)` → `ColumnCounts` | The column's row / NULL / distinct counts under the filter — the numbers above the panel's chart, and what decides which chart it gets. A column whose exact distinct count wants more than the session's memory pool — tens of millions of distinct values, where the count keeps every one of them and cannot spill — is estimated instead (`approx_distinct`) and says so (`distinct_approximate`), which the panel prints as `≈ n`. One scan through the single-partition session (`services/profile.rs`) |
| `profile_column_chart` | `(path, column, filter?, counts, requestId?)` → `ProfileChart` | The chart of the same column and rows, as a second call so the panel can show the counts without waiting for it — on 58M rows the counts land in 0.5–4 s and the chart can take 20. Every value with its count when there are at most 20 distinct values (never on an estimated count, which cannot promise a complete list), else equal-width buckets on round edges (whole days / seconds for temporal columns) for numbers and dates and the 20 commonest values for the rest, with what is not shown counted as `other`. Bucket counts use the same typed predicates as drill-down; day-end time buckets include their final representable instant. Unsafe double ranges and decimals with more than 15 digits of precision fall back to top values. One or two more scans |
