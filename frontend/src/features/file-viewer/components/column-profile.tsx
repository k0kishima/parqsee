...
import { profileColumn, type ColumnInfo, type ColumnProfile, type HistogramBucket, type ValueCount } from '../api';
 ...
 * has under the grid's filter, and a chart of its values — every value
 ...
 * or a bucket adds it to the filter bar, so "see the distribution, then
 * narrow to a slice" is one motion; the panel then profiles the slice.
 *
 * One request per (file, column, filter). A newer request supersedes an
 * older one whatever order they return in — profiling is a scan of the
...
export function ColumnProfilePanel({ filePath, column, filter, onClose, onAddConditions }: ColumnProfilePanelProps) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError(null);
    profileColumn(filePath, columnName, filter || undefined).then(
      result => {
  ...
  return (
    <aside
      aria-label={t('viewer.profile.title', { column: columnName })}
    ...
    >
      <div className="flex items-start gap-2 px-3 py-2 border-b border-primary">
        <div className="min-w-0 flex-1">
          <h2 className="text-sm font-semibold truncate text-primary" title={columnName}>{columnName}</h2>
          <div className="text-xs text-tertiary">{column.column_type}</div>
        </div>
        <button
          type="button"
          onClick={onClose}
